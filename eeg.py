...
class ControllerIO():
    ...
    def onData(self, uid, text):
        ...
        if ioCommand[0] == "CyKITv2":
            ...
            if ioCommand[1] == "getDataMode":
                mirror.text("Sending >>> datamode:::" +
                            str(self.getInfo("datamode")))
            ...
            if ioCommand[1] == "setDataMode":
                ...
                if eval(self.getInfo("verbose")) == True:
                    mirror.text(
                        ">>> Client Setting >>> DataMode = " + str(self.datamode))
                return
            ...
            if ioCommand[1] == "changeFormat":
                ...
                if self.format == 1:
                    ...
                    mirror.text(
                        "Format Change (Format-0): Python handling float conversion.\r\n")
            ...
            if ioCommand[1] == "InfoRequest":
                self.server.sendData(
                    "CyKITv2:::Info:::device:::" + str(self.getInfo(["device"])))
                self.server.sendData(
                    "CyKITv2:::Info:::serial:::" + str(self.getInfo(["serial"])))
            ...
            if ioCommand[1] == "UpdateSettings":
                ...
                if "DeviceObject" in self.infoData and self.getInfo("intf") != None:
                    settings_menu(
                        self.infoData["DeviceObject"], self, self.getInfo("intf"))
                else:
            ...
            if ioCommand[1] == "getBaseline":
                if self.baseline_data != None:
                    self.server.sendData(
                        "CyKITv2:::Baseline:::" + str(self.baseline_data))
            ...
            if ioCommand[1] == "RecordStart":
                if eval(self.getInfo("recording")) == True:
                    self.stopRecord()
                    mirror.text(
                        "[Record Stopped] -- Press 'Record' to Record a new file.")
                    return
                ...
                if os.path.exists(cyPath + "/EEG-Logs") == False:
                    try:
                        ...
                        mirror.text("*** Failed to Create Directory: '" +
                ...
                try:
                    while os.path.exists(pathFinder):
                        self.recordInc += 1
                        self.recordFile = self.recordFile.split(
                            "_")[0] + "_" + str(self.recordInc)
                        pathFinder = cyPath + "/EEG-Logs/" + self.recordFile + ".csv"
                        if eval(self.getInfo("verbose")) == True:
                            mirror.text(
                ...
                mirror.text("[Start] Recording to File: " +
                            self.recordFile + " \r\n")
                try:
                    self.cyFile = open(
                        cyPath + "\\EEG-Logs\\" + self.recordFile + ".csv", "w+", newline='')
                    ...
                    mirror.text(
                        " ¯¯¯¯ eegThread.run() Error Communicating With USB.")
                    mirror.text(
                        " =E.3: " + print_format.format(exc_type.__name__, line_number, ex))
                    return
            ...
            if ioCommand[1] == "setMask":
                try:
                    ...
                    self.setMask[maskSelect] = map(
                        int, str(ioCommand[3]).split(","))
                except Exception as msg:
    ...
    def sendInfo(self, name):
        if name not in self.infoData:
            ...
            return
        self.server.sendData("CyKITv2:::Info:::" +
                             str(name) + ":::" + str(self.infoData[str(name)]))
        return
...
def settings_menu(device, sIO, intf):
    ...
    if eeg_driver == "pyusb":
        ...
        if device_firmware != "0x565" or software_firmware != "0x625":
            ...
            mirror.text(
                "   Please Report your firmware and software to the software developer:")
    ...
    if sIO.getInfo("updateEPOC") != "None":
        ...
        mirror.text("═" * 100)
        mirror.text(
            "  *** Important Advisories *** (Please Read)              ")
        mirror.text("═" * 100)
        mirror.text(
        ...
        mirror.text(
            "  EPOC+ Mode Selection Menu. [Current Mode: " + current_txt + "]")
        mirror.text("═" * 100)
        mirror.text(" 0) EPOC (14-bit mode)                       " +
                    str(current[0]))
        mirror.text(" 1) EPOC+ 128hz 16bit - MEMS off             " +
                    str(current[1]))
        mirror.text(" 2) EPOC+ 128hz 16bit - MEMS 32hz  16bit     " +
                    str(current[2]))
        mirror.text(" 3) EPOC+ 128hz 16bit - MEMS 64hz  16bit     " +
                    str(current[3]))
        mirror.text(" 4) EPOC+ 128hz 16bit - MEMS 128hz 16bit     " +
                    str(current[4]))
        mirror.text(" 5) EPOC+ 256hz 16bit - MEMS off             " +
                    str(current[5]))
        mirror.text(" 6) EPOC+ 256hz 16bit - MEMS 32hz  16bit     " +
                    str(current[6]))
        mirror.text(" 7) EPOC+ 256hz 16bit - MEMS 64hz  16bit     " +
                    str(current[7]))
        mirror.text(" 8) EPOC+ 256hz 16bit - MEMS 128hz 16bit     " +
                    str(current[8]) + "\r\n")
        mode_select = input(
            " Enter Mode: [0,1,2,3,4,5,6,7,8] or [Q] to Exit \> ")
...
class EEG(object):
    ...
    def __init__(self, model, io, config):
        ...
        self.recordInc = 1
        self.thread_1 = threading.Thread(name='eegThread', target=self.run, kwargs={
                                         'key': self.myKey, 'cyIO': self.cyIO}, daemon=False)
        ...
        self.mask[6] = [102, 103, 88, 89, 90,
                        91, 92, 93, 94, 95, 80, 81, 82, 83]
        self.mask[7] = [140, 141, 142, 143, 128, 129,
                        130, 131, 132, 133, 134, 135, 120, 121]
        self.mask[8] = [158, 159, 144, 145, 146, 147,
                        148, 149, 150, 151, 136, 137, 138, 139]
        self.mask[9] = [160, 161, 162, 163, 164, 165,
                        166, 167, 152, 153, 154, 155, 156, 157]
        self.mask[10] = [178, 179, 180, 181, 182, 183,
                         168, 169, 170, 171, 172, 173, 174, 175]
        self.mask[11] = [196, 197, 198, 199, 184, 185,
                         186, 187, 188, 189, 190, 191, 176, 177]
        self.mask[12] = [214, 215, 200, 201, 202, 203,
                         204, 205, 206, 207, 192, 193, 194, 195]
        self.mask[13] = [216, 217, 218, 219, 220, 221,
                         222, 223, 208, 209, 210, 211, 212, 213]

        self.insight_1 = [0, 8, 14, 22, 28, 36, 42, 50, 56, 64, 70, 78, 84, 92, 98, 106, 112, 120,
                          126, 134, 140, 148, 154, 162, 168, 176, 182, 190, 196, 204, 210, 218, 224, 232, 238]
        self.insight_2 = [0, 8, 14, 22, 28, 36, 42, 50, 56, 64]

        self.blank_data = {}
        # BlankData for Epoc??
        self.blank_data[2] = [0, 11, 45, 226, 13, 209, 11, 156, 77, 16, 118, 83, 208, 255, 75,
                              10, 40, 241, 206, 231, 146, 226, 59, 124, 165, 69, 24, 248, 163, 55, 25, 133, 167]
        self.blank_data[6] = [0, 16, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128, 0,
                              128, 0, 128, 0, 0, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128, 0, 128]
    ...
    def Setup(self, model, config):
        ...
        deviceList = ['EPOC+', 'EEG Signals',
                      '00000000000', 'Emotiv RAW DATA', ]
        ...
        if model == 1:
            k = [sn[-1], 00, sn[-2], 72, sn[-1], 00, sn[-2], 84,
                 sn[-3], 16, sn[-4], 66, sn[-3], 00, sn[-4], 80]
            self.samplingRate = 128
            self.channels = 40

        # --- Model 2 > [Epoc::Consumer]
        if model == 2:
            k = [sn[-1], 00, sn[-2], 84, sn[-3], 16, sn[-4], 66,
                 sn[-1], 00, sn[-2], 72, sn[-3], 00, sn[-4], 80]
            self.samplingRate = 128
            self.channels = 40

        # --- Model 3 >  [Insight::Premium]
        if model == 3:
            k = [sn[-2], 00, sn[-1], 68, sn[-2], 00, sn[-1], 12,
                 sn[-4], 00, sn[-3], 21, sn[-4], 00, sn[-3], 88]
            self.samplingRate = 128
            self.channels = 20
    ...
    def run(self, key, cyIO):
        ...
        if eval(cyIO.getInfo("noheader")) == False:
            ...
            if eval(cyIO.getInfo("status")) == True and eval(cyIO.getInfo("noweb")) == False:
                ...
                cyIO.sendData(1, "CyKITv2:::Info:::delimiter:::" +
        ...
        while self.running:
            ...
            if self.blankdata == True:
                try:
                    ...
                    encrypted_blank_cipher = b"0" + \
                        (cipher.encrypt(bytes(join_data, 'latin-1')))
                except Exception as e:
                    mirror.text(
                ...
                try:
                    ...
                    if (int(time.time() % 60) - self.getSeconds) == 1:
                        ...
                        if eval(cyIO.getInfo("status")) != True:
                            ...
                            while "Thread-" in t_array:
                                ...
                                mirror.text(t_array)
                                self.cyIO.getInfo(
                                    "DeviceObject").set_raw_data_handler(None)
                        ...
                        if self.format < 1:
                            for i in range(0, 14):
                                packet_data = packet_data + \
                                    str(self.convertEPOC(
                                        data[1:], self.mask[i])) + self.delimiter
                    ...
                    if self.KeyModel == 4 or self.KeyModel == 7:
                        ...
                        if self.format == 2:
                            ...
                            if self.outputdata == True:
                                if self.channel != None:
                                    mirror.text(str(counter_data) + self.delimiter + str(
                                        packet_data.split(self.delimiter)[int(self.channel)]))
                                else:
                                    mirror.text(
                        ...
                        if self.format < 1:
                            ...
                            if self.nobattery == False:
                                packet_data = packet_data + self.delimiter + \
                                    str(data[16]) + \
                                    str(self.delimiter) + str(data[17])
                    ...
                    if self.KeyModel == 6 or self.KeyModel == 5:
                        ...
                        if self.nocounter == True:
                            ...
                            counter_data = str(
                                data[0]) + self.delimiter + str(data[1]) + self.delimiter
                        ...
                        if self.format < 1:
                            ...
                            if self.baseline == True:
                                ...
                                if (int(time.time() % 60) - self.baseSeconds) == 1:
                                    ...
                                    try:
                                        ...
                                        if baseline_values != None and 'baseline_last' in locals():
                                            baseline_values = list(
                                                map(operator.add, baseline_last, baseline_values))
                                            set_values = (
                                                [2] * len(baseline_values))
                                            baseline_values = list(
                                                map(operator.truediv, baseline_values, set_values))
                                            ...
                                            send_baseline = [0.0, float(
                                                str(data[1]))] + baseline_values
                                        ...
                                        mirror.text(
                                            " =E.7: " + print_format.format(exc_type.__name__, line_number, ex))
                            ...
                            if cyIO.isRecording() == True:
                                ...
                                if self.blankcsv == True:
                                    emptyCSV = (
                                        "0" + self.delimiter) * int(self.channels - (16 + abs((self.nobattery & 1) * -2)))
                        ...
                        if self.openvibe == True:
                            ...
                            if self.filter == True and self.format == 0:
                                if 'baseline_values' in locals():
                                    ...
                                    filter_data = map(
                                        operator.sub, baseline_values, convert_packet)
                    ...
                    mirror.text(
                        " =E.8: " + print_format.format(exc_type.__name__, line_number, ex))
