...
if sys.argv[2].isdigit() == False or int(sys.argv[2]) < 1025 or int(sys.argv[2]) > 65535:
    mirror("Invalid Port #[" + str(sys.argv[2]) +
           "] (Must be a local port in range: 1025 - 65535)")
    os._exit(0)

if sys.argv[3].isdigit() == False:
    mirror("Invalid Key # [" + str(sys.argv[3]) +
           "] (Must be a numeric 1 - 9)")
    os._exit(0)
...
try:
    ...
    mirror("CyKITv2.Main() : " +
           print_format.format(exc_type.__name__, line_number, ex))
