...
class socketIO():
    ...
    def __init__(self, port, uid, ioHandler):
        ...
        self.thread_2 = threading.Thread(
            name='ioThread', target=self.run, daemon=False)
            ...
            if not self.isHandleShake:
                try:
                    ...
                    if ready[0]:
                        ...
                        resKey = base64.encodestring(hashlib.new(
                            "sha1", (secKey+"258EAFA5-E914-47DA-95CA-C5AB0DC85B11").encode('utf-8')).digest())
                    ...
                    mirror.text(print_format.format(
                ...
                try:
                    ...
                    if ready[0]:
                        ...
                        data_lengths = struct.unpack(
                            "B", bytes(data_length, 'latin-1'))
                        ...
                        if data_length <= 125:
                            ...
                            payloadLength = struct.unpack("H", bytes(
                                self.con.recv(2).decode('latin-1'), 'latin-1'))[0]
                        elif data_length == 127:
                            payloadLength = struct.unpack("Q", bytes(
                                self.con.recv(8).decode('latin-1'), 'latin-1'))[0]
    ...
    def onData(self, text):
        try:
            ...
            self.con.close()
        hashStr = hashlib.new(
            "md5", (str(uid)+self.signKey).encode('utf-8')).hexdigest()
        if hashStr != sign:
    ...
    def packData(self, text):
        ...
        sign = hashlib.new(
            "md5", (str(self.uid)+self.signKey).encode('utf-8')).hexdigest()
    ...
    def sendOVint(self, text):
        try:
            ...
            self.ovData += b''.join((struct.pack('>h', val)
                                    for val in ov_ints))
    ...
    def sendOVfloat(self, text):
        try:
            ...
            self.ovData += b''.join((struct.pack('>f', val)
                                    for val in ov_floats))
            self.ov_packetCount += 1
    ...
    def sendData(self, text):
        ...
        if self.uid == 0:
            try:
                ...
                mirror.text(
                    " ==== CyWebSocket().sendData() - (TEXT) Send Data Failure. ")
            ...
            try:
                ...
                self.con.send(struct.pack("!%ds" %
                              (length,), bytes(text, 'utf-8')))
                ...
                print_format = '{}: Exception in line: {}, message: {}'
                mirror.text(
                    " ==== CyWebSocket().sendData() - (WEB CLIENT) Send Data Failure. ")
                mirror.text(
                    " =E4. " + print_format.format(exc_type.__name__, line_number, ex))
                mirror.text(traceback.format_exc())
                return
